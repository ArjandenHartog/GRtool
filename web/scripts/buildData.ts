...
for (const jaar of BESCHIKBARE_JAREN) {
  console.log(`Processing year: ${jaar}`);
  const data = getAllGemeenten(jaar);
  const outPath = path.join(OUT_DIR, `${jaar}.json`);
  fs.writeFileSync(outPath, JSON.stringify(data), 'utf-8');
  console.log(`Saved ${data.length} gemeenten to ${outPath} (${(fs.statSync(outPath).size / 1024 / 1024).toFixed(2)} MB)`);
}
console.log('Done!');
